# -*- coding: utf-8 -*-
"""
Created on Mon May 12 12:17:20 2025
...
#Save channel
# with open('channel_10data.pkl', 'wb') as f:
#     pickle.dump(h_ch, f)

#load channel
# with open('channel.pkl', 'rb') as f:
#     h_channel = pickle.load(f)
...
for i_eps in range(N_eps):
    ...
    loss_min_array = loss_mean_array-np.std(loss_mean_array)
    loss_max_array = loss_mean_array+np.std(loss_mean_array)
    
    rate_mean_array.append(np.mean(rate_array))
    rate_min_array = rate_mean_array-np.std(rate_mean_array)
    rate_max_array = rate_mean_array+np.std(rate_mean_array)
...
# plt.savefig('training_loss_revise_plot_100data.svg', format='svg', dpi=1200, bbox_inches="tight")
plt.show()
...
# plt.savefig('rate_revise_plot_100data.svg', format='svg', dpi=1200, bbox_inches="tight")
plt.show()
...
with open('QGNN_training_enc_VE_revise_100data.pkl', 'wb') as f:
    pickle.dump(fig, f)
...
# === Save the figure object to pickle ===
with open('QGNN_rate_enc_VE_revise_100data.pkl', 'wb') as f:
    pickle.dump(fig, f)
